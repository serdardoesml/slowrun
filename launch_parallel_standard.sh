#!/bin/bash
# Run downstream evals for a train.py checkpoint/artifact across 8 GPUs.
#
# Usage:
#   ./launch_parallel_standard.sh <checkpoint.pt> [tasks] [batch_size] [output_dir] [max_length]
#
# Example:
#   ./launch_parallel_standard.sh runs/my-run/model.pt "hellaswag,arc_easy,sciq,piqa" 8 results_standard 2048
...
{
echo "========================================"
echo "Slowrun Standard Multi-GPU Evaluation"
echo "========================================"
...
for rank in $(seq 0 $((WORLD_SIZE - 1))); do
    ...
    CUDA_VISIBLE_DEVICES=$rank "$PYTHON_BIN" bench_standard.py \
        --rank $rank \
...
if [ $FAILED -eq 0 ]; then
    ...
    "$PYTHON_BIN" bench_standard.py combine "$OUTPUT_DIR"/results_rank*.json \
