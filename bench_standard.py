"""
LM-eval runner for checkpoints saved by ./train.py.

Supports:
  - slowrun_eval_artifact_v1 single_model artifacts
  - slowrun_eval_artifact_v1 logit_ensemble artifacts
  - bare parameter state dicts from logit_avg_ckpts/

The model code below is the train.py inference path with training-only optimizer
and data-loader code removed. Attention uses PyTorch SDPA so this file does not
need FlashAttention at eval time.
"""
import argparse
import glob
import json
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        if self._dupe_layers is None:
            x = self._run_decoder_layers(x, x0, encoder_outputs, self.encoder_layers, self.config.n_layer, T)
        else:
            start, end = self._dupe_layers
            x = self._run_decoder_layers(x, x0, encoder_outputs, self.encoder_layers, end, T)
            for _ in range(self._dupe_loops):
                x = self._run_decoder_layers(x, x0, encoder_outputs, start, end, T)
            x = self._run_decoder_layers(x, x0, encoder_outputs, end, self.config.n_layer, T)
        x = norm(x)
...
def extract_artifact(checkpoint_path):
    ...
    if isinstance(ckpt, dict) and "state_dict" in ckpt and isinstance(ckpt["state_dict"], dict):
        return {"artifact_type": "single_model", "state_dict": ckpt["state_dict"], "model_config": ckpt.get("model_config", {})}
    if isinstance(ckpt, dict) and "model_state_dict" in ckpt:
        return {"artifact_type": "single_model", "state_dict": ckpt["model_state_dict"], "model_config": ckpt.get("model_config", {})}
    if isinstance(ckpt, dict) and "model" in ckpt:
        return {"artifact_type": "single_model", "state_dict": ckpt["model"], "model_config": ckpt.get("model_config", {})}
...
def infer_config(state_dict, model_config, logit_cap):
    ...
    n_head = int(model_config.get("n_head") or state_dict["transformer.h.0.attn.attn_gate.weight"].shape[0])
    head_dim = n_embd // n_head
    c_k_out = state_dict["transformer.h.0.attn.c_k.weight"].shape[0]
    ...
    return GPTConfig(
        ...
        logit_cap=logit_cap,
        mtp_weight=1.0 if "mtp_proj.weight" in state_dict else 0.0,
    )


def should_enable_dupe(artifact, dupe_mode, dupe_start_epoch):
    if dupe_mode == "on":
        return True
    if dupe_mode == "off":
        return False
    ...
    return isinstance(epoch, int) and epoch >= dupe_start_epoch


def load_single_model(state_dict_raw, model_config, device, dtype, logit_cap, dupe):
    state_dict, _ = clean_state_dict(state_dict_raw)
    config = infer_config(state_dict, model_config or {}, logit_cap)
    model = GPT(config)
    if dupe:
        start, end, loops = dupe
        if start >= model.encoder_layers and end <= config.n_layer:
            model.set_dupe_layers(start, end, loops)
        else:
            print(f"Warning: ignoring invalid dupe range {start}:{end} for n_layer={config.n_layer}")
    missing, unexpected = model.load_state_dict(state_dict, strict=False)
    real_missing = [k for k in missing if k not in {"cos", "sin"} and not k.startswith("mtp_") and not k.startswith("mtp_block.")]
    real_unexpected = [k for k in unexpected if not k.startswith("mtp_") and not k.startswith("mtp_block.")]
    if real_missing:
        raise RuntimeError(f"Missing checkpoint keys: {real_missing[:20]}{' ...' if len(real_missing) > 20 else ''}")
    if real_unexpected:
        raise RuntimeError(f"Unexpected checkpoint keys for standard model: {real_unexpected[:20]}{' ...' if len(real_unexpected) > 20 else ''}")
...
def load_models_from_checkpoint(checkpoint_path, device, dtype, logit_cap, dupe_mode, dupe_start_epoch, dupe_layers_start, dupe_layers_end, dupe_loops):
    artifact = extract_artifact(checkpoint_path)
    model_config = artifact.get("model_config") or {}
    dupe = None
    if should_enable_dupe(artifact, dupe_mode, dupe_start_epoch):
        dupe = (dupe_layers_start, dupe_layers_end, dupe_loops)
    if artifact.get("artifact_type") == "logit_ensemble":
        weights = artifact.get("ensemble_weights") or [1.0] * len(artifact["checkpoints"])
        total = float(sum(weights))
        weights = [float(w) / total for w in weights]
        models = [
            load_single_model(item["state_dict"], model_config, device, dtype, logit_cap, dupe)
            for item in artifact["checkpoints"]
        ]
        return models, weights
    model = load_single_model(artifact["state_dict"], model_config, device, dtype, logit_cap, dupe)
    return [model], [1.0]
...
class SlowrunStandardLM(LM):
    def __init__(self, checkpoint, rank=0, world_size=1, device="cuda:0", batch_size=1,
                 max_length=2048, dtype="bfloat16", logit_cap=10.0, dupe_mode="auto",
                 dupe_start_epoch=7, dupe_layers_start=15, dupe_layers_end=21, dupe_loops=2):
        super().__init__()
        self._rank = rank
        ...
        self.models, self.weights = load_models_from_checkpoint(
            checkpoint, self._device, self._dtype, logit_cap, dupe_mode,
            dupe_start_epoch, dupe_layers_start, dupe_layers_end, dupe_loops,
        )
        self._enc = tiktoken.get_encoding("gpt2")
        self._eos_token_id = self._enc._special_tokens["<|endoftext|>"]
        print(f"[Rank {rank}/{world_size}] Loaded {len(self.models)} standard model(s) from {checkpoint}")
    ...
    def loglikelihood_rolling(self, requests: List[Instance]) -> List[float]:
        ...
        for req in tqdm(requests, desc=f"[r{self._rank}] loglikelihood_rolling"):
            ...
            for i in range(1, len(toks)):
                total += log_probs[0, i - 1, toks[i]].item()
...
def eval_main(argv):
    ...
    p.add_argument("--dupe-mode", choices=["auto", "on", "off"], default="auto")
    p.add_argument("--dupe-start-epoch", type=int, default=7)
    p.add_argument("--dupe-layers-start", type=int, default=15)
    p.add_argument("--dupe-layers-end", type=int, default=21)
    p.add_argument("--dupe-loops", type=int, default=2)
    args = p.parse_args(argv)
    tasks = [t.strip() for t in args.tasks.split(",") if t.strip()]
    model = SlowrunStandardLM(
        ...
        dupe_mode=args.dupe_mode,
        dupe_start_epoch=args.dupe_start_epoch,
        dupe_layers_start=args.dupe_layers_start,
        dupe_layers_end=args.dupe_layers_end,
